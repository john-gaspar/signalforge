...
- Benchmark — Entry: `sentinelqa/bench/run.py`; Gate: `sentinelqa/gates/bench_gate.py`; Baseline: `sentinelqa/baselines/bench_baseline.json`; Records bench metrics to `artifacts/bench/history.jsonl`.
- Trend regression — Entry: bench history (`artifacts/bench/history.jsonl`); Gate: `sentinelqa/gates/gate_trend_regression.py`; Baseline: trend thresholds in `sentinelqa/gates/thresholds.yaml`.
...
- Gate ledger + runner (`sentinelqa/gates/runner.py`) executes gates in deterministic order (including trend regression after benchmark), records provenance to `artifacts/runs/<run_id>/gates.json`, and CI invokes it instead of ad-hoc gate calls.
